...
import {
  ...
  setAnsweredIds,
  resetMRState,
} from "@/redux/services/mr/mrSlice";
...
const MRPage: React.FC = () => {
  ...
  } = useSelector((state: RootState) => state.mr);
  ...
  console.log("🎯 [MR Page] Tour conditions:", {
    view,
  ...
  useEffect(() => {
    console.log('🔄 [MR] Answer changed, resetting typing state');
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      console.log("✅ [MR] Redux rehydration complete");
  ...
  useEffect(() => {
    ...
    if (currentProjectStr) {
      try {
        ...
        console.error("❌ [MR Project] Error parsing currentProject:", error);
      }
    }
  }, [dispatch, projectId]);
  ...
  useEffect(() => {
    if (resetTimestamp > 0 && isRehydrated && projectId) {
      console.log("╔════════════════════════════════════════════════════════════╗");
      console.log("║          🔄 MR STATE RESET DETECTED                       ║");
      console.log("╚════════════════════════════════════════════════════════════╝");
      console.log(`📅 [MR Reset] Reset timestamp: ${new Date(resetTimestamp).toISOString()}`);
      console.log(`📦 [MR Reset] Project ID: ${projectId}`);
      
      // Reset all ref flags
      console.log("🧹 [MR Reset] Resetting all ref flags...");
      ...
      console.log("🔄 [MR Reset] Resetting view and indices...");
      ...
      setTimeout(() => {
        console.log("🚀 [MR Reset] Triggering fresh data fetch");
        dispatch(setShouldFetchUnanswered(true));
      }, 200);
      
      console.log("✅ [MR Reset] Re-initialization complete");
    }
  ...
  const refetchQuestionsOnReturn = useCallback(() => {
    if (isRefetchingQuestions.current) {
      console.log("⏸️ [MR Refetch] Already refetching, skipping");
      return;
    }

    console.log(
      "🔄 [MR Refetch] User returned to questions view - refetching unanswered questions"
    );
  ...
  useEffect(() => {
    ...
    const handleVisibilityChange = () => {
      if (!document.hidden && view === "questions" && questions.length > 0) {
        console.log(
          "👁️ [MR Visibility] User returned to tab - refetching questions"
        );
  ...
  useEffect(() => {
    ...
    if (view === "questions" && questions.length > 0) {
      ...
      if (hasNavigatedBack) {
        console.log(
          "🔄 [MR Mount] Detected return to questions view - refetching"
        );
        refetchQuestionsOnReturn();
  ...
  useEffect(() => {
    return () => {
      console.log("🧹 [MR Unmount] Resetting fetch flag");
  ...
  useEffect(() => {
    ...
    console.log("🚀 [MR Mount] Running ONE-TIME mount fetch");
  ...
  useEffect(() => {
    if (shouldFetchUnanswered && projectId && isRehydrated) {
      console.log("🔄 [MR] Manually triggering unanswered questions refetch");
      refetchUnanswered();
    }
  ...
  useEffect(() => {
    if (shouldFetchAll && projectId && isRehydrated) {
      console.log("🔄 [MR] Manually triggering all questions refetch");
      refetchAllQuestions();
    }
  ...
  useEffect(() => {
    if (questions.length > 0 && currentQuestionIndex >= questions.length) {
      console.log(
        "⚠️ [MR Safety] currentQuestionIndex out of bounds, resetting to 0"
      );
      dispatch(setCurrentQuestionIndex(0));
    }
  ...
  const handleDocumentDownload = useCallback(async () => {
    ...
    try {
      ...
      console.log("📥 [MR] Downloading generated document...");
      ...
      dispatch(
        setDocumentData({
          ...
          fileName: response.fileName || "mr_document.docx",
        })
      );
      ...
      console.log("✅ [MR] Document downloaded successfully");
    } catch (error: any) {
      console.error("❌ [MR Document] Download failed:", error);
      toast.dismiss("download-doc");
  ...
  useEffect(() => {
    if (hasReceivedCompletionMessage && !showDocumentPreview && !docxBase64) {
      console.log(
        "✅ [MR] Generation complete - triggering document download"
      );
  ...
  useEffect(() => {
    if (
      ...
      console.log("⚠️ [MR Safety] 100% reached, forcing completion");
      setTimeout(() => {
        dispatch(setCompletionMessageReceived(true));
      }, 2000);
    }
  ...
  useEffect(() => {
    ...
    console.log(
      "📥 [MR API Response] Unanswered questions received:",
      unansweredData
    );
  ...
  useEffect(() => {
    ...
    console.log(
      "📥 [MR API Response] All questions received:",
      allQuestionsData
    );
    ...
    console.log(
      `✅ [MR] Loaded ${formattedQuestions.length} answered questions`
    );
    dispatch(setQuestions(formattedQuestions));
  ...
  const handleShowPreview = useCallback(() => {
    console.log("📋 [MR Preview] Triggering API fetch for preview");
    dispatch(setShouldFetchAll(true));
  }, [dispatch]);
  ...
  const handleTypingComplete = useCallback(() => {
    console.log('✅ [MR] Typing animation complete');
  ...
  const handleGenerateDocument = async () => {
    try {
      ...
      if (!baseWsUrl) {
        console.error(
          "❌ [MR] WebSocket URL not configured in environment variables"
        );
        toast.error(
      ...
      console.log(
        "║          🚀 STARTING MR DOCUMENT GENERATION               ║"
      );
      console.log(
        "╚════════════════════════════════════════════════════════════╝"
      );
      console.log("🔌 [MR] Base WebSocket URL:", baseWsUrl);
      console.log("🔌 [MR] Full WebSocket URL:", websocketUrl);
      console.log(
        "🔑 [MR] Session Token:",
        savedToken ? "✅ Present" : "❌ Missing"
      );
      console.log("📦 [MR] Project ID:", project_id);
      console.log("📦 [MR] Dispatching Redux actions...");

      dispatch(setWsUrl(websocketUrl));
      console.log("✅ [MR] wsUrl dispatched to Redux");

      await new Promise((resolve) => setTimeout(resolve, 100));

      dispatch(setIsGenerating(true));
      console.log("✅ [MR] isGenerating=true dispatched to Redux");
      console.log(
        "⏳ [MR] Waiting for middleware to establish WebSocket connection..."
      );
    } catch (err: any) {
      console.error("❌ [MR Upload] Error:", err);
  ...
  return (
    ...
    >
      {isGenerating ? (
        ...
        >
          <Generating wsUrl={wsUrl} documentType="mr" />
        </Box>
      ) : (
        <>
          {view === "questions" &&
