...
import {
  ...
  setAnsweredIds,
  resetBrandState,
} from "@/redux/services/brand/brandSlice";
...
const BrandPage: React.FC = () => {
  ...
  console.log("🎯 [Brand Page] Tour conditions:", {
    view,
  ...
  useEffect(() => {
    console.log('🔄 [Brand] Answer changed, resetting typing state');
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      console.log("✅ [Brand] Redux rehydration complete");
  ...
  useEffect(() => {
    ...
    if (currentProjectStr) {
      try {
        ...
        console.error("❌ [Brand Project] Error parsing currentProject:", error);
      }
    }
  }, [dispatch, projectId]);
  ...
  useEffect(() => {
    if (resetTimestamp > 0 && isRehydrated && projectId) {
      console.log("╔════════════════════════════════════════════════════════════╗");
      console.log("║          🔄 BRAND STATE RESET DETECTED                    ║");
      console.log("╚════════════════════════════════════════════════════════════╝");
      console.log(`📅 [Brand Reset] Reset timestamp: ${new Date(resetTimestamp).toISOString()}`);
      console.log(`📦 [Brand Reset] Project ID: ${projectId}`);
      
      // Reset all ref flags
      console.log("🧹 [Brand Reset] Resetting all ref flags...");
      ...
      console.log("🔄 [Brand Reset] Resetting view and indices...");
      ...
      setTimeout(() => {
        console.log("🚀 [Brand Reset] Triggering fresh data fetch");
        dispatch(setShouldFetchUnanswered(true));
      }, 200);
      
      console.log("✅ [Brand Reset] Re-initialization complete");
    }
  ...
  const refetchQuestionsOnReturn = useCallback(() => {
    if (isRefetchingQuestions.current) {
      console.log("⏸️ [Brand Refetch] Already refetching, skipping");
      return;
    }

    console.log(
      "🔄 [Brand Refetch] User returned to questions view - refetching unanswered questions"
    );
  ...
  useEffect(() => {
    ...
    const handleVisibilityChange = () => {
      if (!document.hidden && view === "questions" && questions.length > 0) {
        console.log(
          "👁️ [Brand Visibility] User returned to tab - refetching questions"
        );
  ...
  useEffect(() => {
    ...
    if (view === "questions" && questions.length > 0) {
      ...
      if (hasNavigatedBack) {
        console.log(
          "🔄 [Brand Mount] Detected return to questions view - refetching"
        );
        refetchQuestionsOnReturn();
  ...
  useEffect(() => {
    return () => {
      console.log("🧹 [Brand Unmount] Resetting fetch flag");
  ...
  useEffect(() => {
    ...
    console.log("🚀 [Brand Mount] Running ONE-TIME mount fetch");
  ...
  useEffect(() => {
    if (shouldFetchUnanswered && projectId && isRehydrated) {
      console.log("🔄 [Brand] Manually triggering unanswered questions refetch");
      refetchUnanswered();
    }
  ...
  useEffect(() => {
    if (shouldFetchAll && projectId && isRehydrated) {
      console.log("🔄 [Brand] Manually triggering all questions refetch");
      refetchAllQuestions();
    }
  ...
  useEffect(() => {
    if (questions.length > 0 && currentQuestionIndex >= questions.length) {
      console.log(
        "⚠️ [Brand Safety] currentQuestionIndex out of bounds, resetting to 0"
      );
      dispatch(setCurrentQuestionIndex(0));
    }
  ...
  const handleDocumentDownload = useCallback(async () => {
    ...
    try {
      ...
      console.log("📥 [Brand] Downloading generated document...");
      ...
      dispatch(
        setDocumentData({
          ...
          fileName: response.fileName || "brand_document.docx",
        })
      );
      ...
      console.log("✅ [Brand] Document downloaded successfully");
    } catch (error: any) {
      console.error("❌ [Brand Document] Download failed:", error);
      toast.dismiss("download-doc");
  ...
  useEffect(() => {
    if (hasReceivedCompletionMessage && !showDocumentPreview && !docxBase64) {
      console.log(
        "✅ [Brand] Generation complete - triggering document download"
      );
  ...
  useEffect(() => {
    if (
      ...
      console.log("⚠️ [Brand Safety] 100% reached, forcing completion");
      setTimeout(() => {
        dispatch(setCompletionMessageReceived(true));
      }, 2000);
    }
  ...
  useEffect(() => {
    ...
    console.log(
      "📥 [Brand API Response] Unanswered questions received:",
      unansweredData
    );
  ...
  useEffect(() => {
    ...
    console.log(
      "📥 [Brand API Response] All questions received:",
      allQuestionsData
    );
    ...
    console.log(
      `✅ [Brand] Loaded ${formattedQuestions.length} answered questions`
    );
    dispatch(setQuestions(formattedQuestions));
  ...
  const handleShowPreview = useCallback(() => {
    console.log("📋 [Brand Preview] Triggering API fetch for preview");
    dispatch(setShouldFetchAll(true));
  }, [dispatch]);
  ...
  const handleTypingComplete = useCallback(() => {
    console.log('✅ [Brand] Typing animation complete');
  ...
  const handleGenerateDocument = async () => {
    try {
      ...
      if (!baseWsUrl) {
        console.error(
          "❌ [Brand] WebSocket URL not configured in environment variables"
        );
        toast.error(
      ...
      console.log(
        "║          🚀 STARTING BRAND DOCUMENT GENERATION            ║"
      );
      console.log(
        "╚════════════════════════════════════════════════════════════╝"
      );
      console.log("🔌 [Brand] Base WebSocket URL:", baseWsUrl);
      console.log("🔌 [Brand] Full WebSocket URL:", websocketUrl);
      console.log(
        "🔑 [Brand] Session Token:",
        savedToken ? "✅ Present" : "❌ Missing"
      );
      console.log("📦 [Brand] Project ID:", project_id);
      console.log("📦 [Brand] Dispatching Redux actions...");

      dispatch(setWsUrl(websocketUrl));
      console.log("✅ [Brand] wsUrl dispatched to Redux");

      await new Promise((resolve) => setTimeout(resolve, 100));

      dispatch(setIsGenerating(true));
      console.log("✅ [Brand] isGenerating=true dispatched to Redux");
      console.log(
        "⏳ [Brand] Waiting for middleware to establish WebSocket connection..."
      );
    } catch (err: any) {
  ...
  return (
    ...
    >
      {isGenerating ? (
        ...
        >
          <Generating wsUrl={wsUrl} documentType="brand" />
        </Box>
      ) : (
        <>
          {view === "questions" &&
