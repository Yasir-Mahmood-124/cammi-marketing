...
const KMFPage: React.FC = () => {
  ...
  console.log("🎯 [KMF Page] Tour conditions:", {
    view,
  ...
  useEffect(() => {
    console.log('🔄 [KMF] Answer changed, resetting typing state');
    setIsTypingComplete(false);
  }, [currentQuestion?.answer, currentQuestionIndex]);

  useEffect(() => {
    console.log('🎯 [KMF Tour Debug]', {
      hasAnswer,
      isTypingComplete,
      readyForRegenerateStep,
  ...
  const handleTypingComplete = useCallback(() => {
    console.log('✅ [KMF] Typing animation complete');
  ...
  const handleGenerateDocument = async () => {
    try {
      ...
      console.log(
        "║          🚀 STARTING KMF DOCUMENT GENERATION              ║"
      );
      console.log(
        "╚════════════════════════════════════════════════════════════╝"
      );
      ...
      console.log("✅ [KMF] wsUrl dispatched to Redux");

      await new Promise((resolve) => setTimeout(resolve, 100));

      dispatch(setIsGenerating(true));
      console.log("✅ [KMF] isGenerating=true dispatched to Redux");
      console.log(
        "⏳ [KMF] Waiting for middleware to establish WebSocket connection..."
      );
    } catch (err: any) {
