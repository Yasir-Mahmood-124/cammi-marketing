  ...
  if (setIsGenerating.match(action) && action.payload === true) {
    ...
    if (wsUrl && wsUrl.includes('4iqvtvmxle') && isGenerating) {
      ...
      if (unsubscribeMessage) {
        console.log("🧹 [SR Global WS] Cleaning up previous listeners");
        unsubscribeMessage();
        unsubscribeError?.();
        unsubscribeClose?.();
      }
      ...
      setTimeout(() => {
        console.log("🔌 [SR Global WS] Connecting to WebSocket...");
      ...
      unsubscribeMessage = wsManager.onMessage((message) => {
        try {
          ...
          if (
            ...
            console.log("╚════════════════════════════════════════════════════════════╝");
            console.log("💾 [SR Global WS] Saving completion flag to Redux (will persist)");
      ...
      unsubscribeClose = wsManager.onClose((event) => {
        console.log("🔗 [SR Global WS] WebSocket closed - Code:", event.code, "Reason:", event.reason);
      });
    }
  }
