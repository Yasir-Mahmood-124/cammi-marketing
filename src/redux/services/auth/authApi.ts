...
export const authApi = createApi({
  reducerPath: "authApi",
  baseQuery: customBaseQuery, // ✅ use custom base query here
  endpoints: (builder) => ({
    ...
    verifyEmail: builder.mutation<
      { message: string; user: { id: string; email: string; firstName: string; lastName: string } },
    ...
    logout: builder.mutation<{ message: string }, { token: string }>({
      query: ({ token }) => ({ url: "/auth/logout", method: "POST", body: { token } }),
    }),
    ...
    verifyCode: builder.mutation<{ message: string }, { email: string; code: string }>({
